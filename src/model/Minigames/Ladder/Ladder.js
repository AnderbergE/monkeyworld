 ...
 * @extends {MW.Minigame}
 */
MW.LadderMinigame = MW.Minigame.extend(
/** @lends {MW.LadderMinigame.prototype} */
{
	/**
	 * @constructs
	...
	init: function (parent) {
		...
		function allowInterrupt(callback) {
			interruptable = true;
			if (ladder.modeIsAgentDo() && !ladder.agentIsInterrupted()) {
		...
		function checkEndOfRound() {
			...
			if (hasMaxTreats || (enoughTries && enoughTreats)) {
				ladder.tellWait(MW.Event.MG_LADDER_CHEER, ladder.quit);
			} else {
				placeTreat();
			}
		}
		...
		this.openTreat = function () {
			collectedTreats += 1;
			if (!ladder.modeIsChild()) {
				ladder.tell(MW.Event.MG_LADDER_CONFIRM_TARGET);
				ladder.addWaterDrop(checkEndOfRound);
			} else {
		...
		this.pick = function (number) {
			...
			Utils.chain(
				...
				function (next) {
					...
					if ((ladder.modeIsAgentDo() && !ladder.agentIsBeingHelped()) ||
		...
		this.addAgentInterruptedHandler(function () {
			if (interruptable && ladder.modeIsAgentDo()) {
				disallowInterrupt();
				ladder.popAction();
				tries -= 1;
				if (!birdHasTreat) {
