...
{
	/** @constructs */
	init: function (stage, useViews, useAgentChooser, startGame) {
		this._super("Game");
		...
		/** @const @type {MW.GamerPlayer}         */ var GAMER           = new MW.GamerPlayer();
		/** @const @type {MW.AgentPlayer}        */ var AGENT           = new MW.AgentPlayer();
...
useAgentChooser = false;
	...
	//		gameMode = MW.GameMode.AGENT_SEE;
			...
			chooseMiniGame(function() {
				if (useViews) {
					miniGameHandlerView = new MW.MinigameHandlerView(stage, that);
		...
		var chooseMiniGame = function(callback) {
			if (minigameArray.length > 0) {
				...
				miniGameStarter = function() {
					miniGame = new configuration.game(that);
		...
		var startMiniGame = function() {
			...
			miniGame.setup();
			if (useViews) {
				currentMiniGameView = new miniGameView(miniGame, stage, agentView);
		...
		var waterGarden = function (callback) {
			...
			if (useViews) {
				var gardenView = new MW.GardenView(stage, that);
				gardenView.setup();
		...
		var playIntroduction = function (callback) {
			if (useViews && !MW.GlobalSettings.debug) {
				var introductionView = new MW.IntroductionView(
					stage,
					function (skipGarden) {
						if (!skipGarden) {
		...
		this.start = function() {
			...
			playIntroduction(/* then */function () {
				if (useAgentChooser) {
					...
					if (useViews) {
						chooserView = new MW.AgentChooserView(stage, chooser);
						chooserView.setup();
						that.tell("Game.viewInitiated");
					}
				} else {
